import { X, Loader2, MonitorPlay } from 'lucide-react'
import { useState, useEffect } from 'react'

// Define Props Interface
interface VideoModalProps {
  trackName: string
  artistName: string
  onClose: () => void
  videoId?: string | null // Optional prop
}

const VideoModal = ({
  trackName,
  artistName,
  onClose,
  videoId: hardcodedVideoId = null
}: VideoModalProps) => {
  const [dynamicVideoId, setDynamicVideoId] = useState<string | null>(null)
  const [loading, setLoading] = useState<boolean>(true)
  const [error, setError] = useState<string | null>(null)
  const [finalVideoId, setFinalVideoId] = useState<string | null>(hardcodedVideoId)
  const [retryCount, setRetryCount] = useState(0)

  useEffect(() => {
    const searchYouTubeVideo = async () => {
      if (hardcodedVideoId) {
        setFinalVideoId(hardcodedVideoId)
        setLoading(false)
        return
      }

      if (!trackName || !artistName) {
        setError('No track info provided')
        setLoading(false)
        return
      }

      setLoading(true)
      setError(null)

      try {
        const query = `${trackName} ${artistName} official music video`
        // Using Piped API to find the video ID without an API key
        const res = await fetch(
          `https://pipedapi.kavin.rocks/search?q=${encodeURIComponent(query)}&filter=videos`
        )

        if (!res.ok) throw new Error('Search failed')

        const data = await res.json()

        if (data.items?.length > 0) {
          // Piped URLs look like /watch?v=VIDEO_ID
          const url = data.items[0].url
          const id = url.split('v=')[1]?.split('&')[0]
          if (id) {
            setDynamicVideoId(id)
            setFinalVideoId(id)
          } else {
            setError('Invalid video URL')
          }
        } else {
          setError('No video found')
        }
      } catch (err) {
        console.error('YouTube search failed:', err)
        setError('Failed to find video')
      } finally {
        setLoading(false)
      }
    }

    searchYouTubeVideo()
  }, [trackName, artistName, hardcodedVideoId])

  // FIXED: Dynamic origin detection + youtube-nocookie.com + referrerPolicy
  const getEmbedUrl = (id: string) => {
    const isElectron = typeof window !== 'undefined' && (window as any).process?.type === 'renderer'
    let originParam = ''

    if (isElectron) {
      // For Electron (file://), use self-referential origin (allowed by YouTube)
      originParam = '&origin=https://www.youtube.com'
    } else {
      // For web/dev, use current origin (localhost or deployed)
      const currentOrigin = window.location.origin
      if (currentOrigin !== 'null') {
        // Avoid file:// fallback
        originParam = `&origin=${encodeURIComponent(currentOrigin)}`
      }
    }

    return `https://www.youtube.com/embed/${id}?autoplay=1&controls=1&fs=1&modestbranding=1`
  }

  const handleRetry = () => {
    setError(null)
    setRetryCount((prev) => prev + 1)
    // Re-run search on retry
    if (hardcodedVideoId) {
      setFinalVideoId(hardcodedVideoId)
    } else {
      // Reset to trigger useEffect
      setDynamicVideoId(null)
      setFinalVideoId(null)
    }
  }

  if (loading) {
    return (
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/95 backdrop-blur-md p-4">
        <div className="relative w-full max-w-5xl bg-black rounded-2xl overflow-hidden shadow-2xl">
          <div className="relative w-full pt-[56.25%] bg-black">
            <div className="absolute inset-0 flex flex-col items-center justify-center text-white">
              <Loader2 className="h-12 w-12 animate-spin text-red-500 mb-4" />
              <p className="text-lg">Finding video...</p>
            </div>
          </div>
        </div>
      </div>
    )
  }

  return (
    <div
      className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/95 backdrop-blur-md p-4"
      onClick={onClose}
    >
      <div
        className="relative w-full max-w-5xl bg-black rounded-2xl overflow-hidden shadow-2xl border border-zinc-800"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-50 flex h-10 w-10 items-center justify-center rounded-full bg-black/70 text-white hover:bg-red-600 transition-all backdrop-blur-sm shadow-lg"
          aria-label="Close video"
        >
          <X size={24} strokeWidth={2.5} />
        </button>

        {/* 16:9 Responsive Container */}
        <div className="relative w-full pt-[56.25%] bg-zinc-900">
          {error ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-zinc-400 p-8">
              <MonitorPlay size={64} className="mb-4 opacity-50" />
              <p className="text-lg text-center mb-4">{error}</p>
              <div className="flex gap-2">
                <button
                  onClick={handleRetry}
                  className="px-6 py-2 bg-zinc-800 hover:bg-zinc-700 rounded-lg transition text-white"
                >
                  Retry ({retryCount + 1})
                </button>
                <button
                  onClick={onClose}
                  className="px-6 py-2 bg-red-600 hover:bg-red-500 rounded-lg transition text-white"
                >
                  Close
                </button>
              </div>
            </div>
          ) : finalVideoId ? (
            <iframe
              key={`${finalVideoId}-${retryCount}`} // Key change forces reload on retry
              className="absolute top-0 left-0 w-full h-full rounded-lg"
              src={getEmbedUrl(finalVideoId)}
              title={`${trackName || 'Music Video'} - YouTube`}
              referrerPolicy="strict-origin-when-cross-origin" // FIXED: Key for Error 153/CORS
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
              allowFullScreen
              loading="lazy"
              onError={(e) => {
                console.error('Iframe load error:', e)
                setError('Video failed to load. Check console.')
              }}
            />
          ) : null}
        </div>
      </div>
    </div>
  )
}

export default VideoModal
